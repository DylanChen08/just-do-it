...
class LinkedList {
    ...
    removeByValue(value) {
        // ✅ dummy 节点作为“安全缓冲头”，可以统一处理删除头节点和中间节点
        // dummy.next 指向原来的链表头部
        const dummy = new ListNode(0);
        dummy.next = this.head;
    
        // ✅ current 表示遍历指针，初始设为 dummy
        // 这样我们可以检查 current.next 是否需要被删除
        let current = dummy;
        ...
        while (current.next) {
            ...
            if (current.next.value === value) {
                ...
                this.length--;
                // ✅ 由于只删一个，找到后就返回
                // 若要删除所有匹配的值，可移除此 return
                this.head = dummy.next;  // 🔁 记得更新真正的 head
                return true;
            }
            current = current.next;
        }
